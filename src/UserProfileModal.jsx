...
export default function UserProfileModal({
  ...
  // NEW: State to control the fullscreen image lightbox
  ...
  return (
    <>
              ...
              <>
                ...
                <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                  <div className="relative">
                    <div 
                      className="w-20 h-20 rounded-full p-1 bg-white shadow-md ring-2 ring-slate-100 group cursor-zoom-in"
                      onClick={() => setEnlargedImage(avatarSource)}
                    >
                      <img
                        src={avatarSource}
                        alt={targetUsername}
                        onError={() => setImageError(true)}
                        className="w-full h-full rounded-full object-cover bg-slate-100 group-hover:opacity-80 transition-opacity"
      ...
      {enlargedImage && (
        <div 
          className="fixed inset-0 z-[9999999] bg-slate-950/95 backdrop-blur-sm flex items-center justify-center p-4 cursor-zoom-out animate-fade-in"
        ...
        >
          <div className="relative max-w-full max-h-full flex flex-col items-center">
            ...
            <button
              ...
              className="absolute -top-12 right-0 md:-right-12 p-2 text-slate-300 hover:text-white bg-slate-800/50 hover:bg-slate-700 rounded-full transition-all cursor-pointer"
              title="Close Image"
            >
              <X className="w-6 h-6" />
            </button>
            ...
            <img
              ...
              className="max-w-[90vw] max-h-[85vh] object-contain rounded-2xl shadow-2xl ring-4 ring-white/10 cursor-default"
              onClick={(e) => e.stopPropagation()} // Prevents closing if clicking directly ON the image
            />
          </div>
        </div>
      )}
